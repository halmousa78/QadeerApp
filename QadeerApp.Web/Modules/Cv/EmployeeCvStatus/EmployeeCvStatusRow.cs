...
[DisplayName("CV Completion Report"), InstanceName("CV Status")]
...
public sealed class EmployeeCvStatusRow : Row<EmployeeCvStatusRow.RowFields>, IIdRow, INameRow
{
    ...
    [DisplayName("Full Name"), Size(100), QuickSearch, NameProperty]
    public string DisplayName { get => fields.DisplayName[this]; set => fields.DisplayName[this] = value; }

    [DisplayName("Active"), Expression("T0.[IsActive]")]
    ...
    [DisplayName("Last Updated"), Expression("coalesce(cv.[UpdateDate], cv.[InsertDate])")]
    public DateTime? CvUpdatedOn { get => fields.CvUpdatedOn[this]; set => fields.CvUpdatedOn[this] = value; }

    [DisplayName("Qualifications Count"), Expression("(select count(1) from EmployeeQualifications q where q.EmployeeCvId = cv.EmployeeCvId)")]
    public int? QualificationCount { get => fields.QualificationCount[this]; set => fields.QualificationCount[this] = value; }

    [DisplayName("Experiences Count"), Expression("(select count(1) from EmployeeExperiences e where e.EmployeeCvId = cv.EmployeeCvId)")]
    public int? ExperienceCount { get => fields.ExperienceCount[this]; set => fields.ExperienceCount[this] = value; }

    [DisplayName("Courses Count"), Expression("(select count(1) from EmployeeCourses c where c.EmployeeCvId = cv.EmployeeCvId)")]
    public int? CourseCount { get => fields.CourseCount[this]; set => fields.CourseCount[this] = value; }

    [DisplayName("Department"), ForeignKey(typeof(DepartmentRow)), LeftJoin("d"), Insertable(false), Updatable(false)]
    public int? DepartmentId { get => fields.DepartmentId[this]; set => fields.DepartmentId[this] = value; }

    [DisplayName("Specialization"), ForeignKey(typeof(SpecializationRow)), LeftJoin("s"), Insertable(false), Updatable(false)]
