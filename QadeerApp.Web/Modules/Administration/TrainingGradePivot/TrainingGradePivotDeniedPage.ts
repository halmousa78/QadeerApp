...
export default () => {
    ...
    function toArabicRows(items: TrainingGradePivotRow[]) {
        const filtered = (items || []).filter(x => (x.RegistrationStatus ?? "").toString().trim() !== "" || (x.TraineeStatus ?? "").toString().trim() !== "");
        return filtered.map(x => ({
            "الفصل التدريبي": x.TrainingTerm || "",
            "حالة التسجيل": x.RegistrationStatus || "",
            "حالة المتدرب": x.TraineeStatus || "",
            "القسم": x.Department || "",
    ...
    function renderPivot(rows: TrainingGradePivotRow[]) {
        ensurePivotLibs().then(() => {
            ...
            $host.pivotUI(data, {
                rows: ["الفصل التدريبي"],
                cols: ["حالة التسجيل", "حالة المتدرب"],
    ...
    function exportCsv() {
        ...
        ensureXlsx().then(() => {
            ...
            XLSX.writeFile(wb, "training-grades-withdrawn.xlsx");
